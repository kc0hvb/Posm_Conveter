...
namespace PicAx_To_IT_Converter {
    ...
    public class ConverterLogic {
                ...
                try {
                    ...
                    using (OleDbConnection passProtConn = new OleDbConnection(returnString)) {
                        ...
                        try {
                            ...
                            Console.WriteLine("Could not open database--with or without PicAx password.");
                            return input;
                        }
                    }
                }
        ...
        public void BeginConversion()
        {
                ...
                insertPosmRowsIntoITDatabase(posmAssetInspectionTable, "MH");
                insertPosmRowsIntoITDatabase(posmAssetInspectionTable, "MHI");
                insertPosmRowsIntoITDatabase(posmObservationTable, "MHO");
                ...
                GeneralStatusUpdate?.Invoke(this, new GeneralConversionNotificationEventArgs("Fixing IsImperial in ITpipes database..."));
        ...
        private void linkChildParentRowsInITpipesDatabase() {
            ...
            using (OleDbCommand curCommand = curConn.CreateCommand()) {
                ...
                curCommand.CommandText = queryText =
                    "UPDATE MHO LEFT JOIN MHI ON MHO.ParentGUID = MHI.ConversionGUID SET MHO.MHI_ID = MHI.MHI_ID;";
                ...
                curCommand.CommandText = queryText = 
                    "UPDATE MHI LEFT JOIN MH ON MHI.ConversionGUID = MH.ConversionGUID SET MHI.MH_ID = MH.MH_ID;"; //in this instance, the conversionGUID is also the parent guid since it comes from the same row.
                ...
                curCommand.CommandText = queryText = 
                    "INSERT INTO MHO_Media (MHO_ID, Media_ID) SELECT MHO_ID, Media_ID FROM MHO LEFT JOIN Media ON MHO.ConversionGUID = Media.ParentGUID WHERE Media.Media_ID IS NOT NULL";
                ...
                curCommand.CommandText = queryText =
                    "INSERT INTO MHI_Media (MHI_ID, Media_ID) SELECT MHI.MHI_ID, Media_ID FROM MHI LEFT JOIN Media ON MHI.ConversionGUID = Media.ParentGUID WHERE Media.Media_ID IS NOT NULL";
        ...
        private void fixingImperialRowsIntoITPipesDatabase(DataTable posmMedia)
        {
            ...
            using (OleDbCommand curCommand = curConn.CreateCommand())
            {
                ...
                curCommand.CommandText = $"UPDATE MHI SET IsImperial = 1;";
        ...
        private void insertPosmRowsIntoITDatabase(DataTable posmTable, string itpipesTable) {
            ...
            string query = "SELECT * FROM [Session] WHERE TemplateName = 'Nassco MACP';";
            ...
            using (OleDbCommand curCommand = curConn.CreateCommand()) {
                ...
                foreach (DataRow curRow in posmTable.Rows) {
                    ...
                    try {
                        curCommand.ExecuteNonQuery();
                        if (itpipesTable.Equals("MHI", StringComparison.InvariantCultureIgnoreCase)) {
        ...
        private void loadCommandWithInsertLogic(DataRow curRow, string itpipesTableName, OleDbCommand commandToLoad) {
            try {
                ...
                foreach (string curField in curFieldMapDictionary.Keys) {
                    ...
                    var vFieldType = value.GetType();
                    string sTest = value.ToString();
                    //Apparently if the value is Yes versus true or 1 then it fires an error about datatype mismatch.
                    bool bIsNumbers = true;
                    ...
                    int iValueNumbers = 0;
                    if (bIsNumbers == true)
                        if (Int32.TryParse(value.ToString(), out iValueNumbers))
                            value = iValueNumbers;
        ...
        private DataTable getObservationsTableFromPosm() {
            ...
            string queryText =
                ...
                "WHERE ParentGUID IN (Select ConversionGUID FROM [Session] WHERE TemplateName = 'Nassco MACP') AND [Data].FaultCodeID <> 1;";
        ...
        private DataTable getInspectionsTableFromPosm() {
            ...
            string queryText =
                "SELECT * FROM [Session] WHERE TemplateName = 'Nassco MACP';";
